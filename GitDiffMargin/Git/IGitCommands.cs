...
namespace GitDiffMargin.Git
{
    ...
    {
        IEnumerable<HunkRangeInfo> GetGitDiffFor(ITextDocument textDocument, string originalPath, ITextSnapshot snapshot);
        ...
        /// </summary>
        /// <remarks>
        /// <para>In some (detectable) cases, documents are opened through a temporary copy of the file rather than
        /// opening the original file directly. To preserve Git diff functionality in these cases, the original path is
        /// detected and used for diff operations.</para>
        /// </remarks>
        /// <param name="path">The path of a document opened in the editor.</param>
        /// <param name="originalPath">The original path of the document in a source-controlled working copy; otherwise,
        /// this is just set to <paramref name="path"/>.</param>
        /// <returns><see langword="true"/> if the resulting <paramref name="originalPath"/> was located (even if it
        /// matches <paramref name="path"/>; otherwise, <see langword="false"/>.</returns>
